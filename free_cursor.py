...
class Config:
    """配置常量类"""
    # 请填写你的用户key,找作者免费获取 https://gitee.com/tackok/free_cursor
    API_URL = "https://mycursor.5f9miy.tech/get-token"
    USER_KEY = ""
    ...
    def fetch_token_data(access_code: str) -> Optional[TokenData]:
        ...
        try:
            response = requests.post(
                ...
                data=json.dumps({"user_key": access_code}),
                timeout=10
            )

            if response.status_code != 200:
                logger.warning(f"API请求失败: 状态码 {response.status_code}")
                logger.warning(f"API请求失败: 原因 {response.content}")
                return None
            data = response.json()
            if not all(key in data for key in ["email", "token"]):
                logger.warning(f"请注意异常f{response.text}，请联系开发者")
                logger.info("联系作者 https://gitee.com/tackok/free_cursor")
                return None
            # 请勿修改服务器提供以下设备信息，因为服务端通过以下信息验证账号有效性，修改后可能会导致账号风控被封
            tokens = TokenData.from_dict({
                "email": data["email"],
                "token": data["token"],
                "mac_machine_id": data["mac_machine_id"],
                "machine_id": data["machine_id"],
            ...
            logger.warning(f"请注意异常f{e}，请联系开发者")
            logger.info("联系作者 https://gitee.com/tackok/free_cursor")
            return None
...
class UserInterface:
    ...
    def request_access_code() -> str:
        ...
        print("\n" + "-" * 40)
        print("请输入授权码")
        print("获取地址：https://mycursor.5f9miy.tech")
        print("-" * 40)
...
def main() -> None:
    ...
    try:
        ...
        time.sleep(0.05)
        logger.error("这个是一个免费项目对外提供，如果你是付费获取到的，请往死里骂！")
        ...
        logger.info("\n")
        logger.warning("注意，先确认之前重置是否成功,或者文件不存在，或者没有权限，未成功请重试，否则替换Token会浪费提取次数")
        ...
        if not Utils.get_user_confirmation("是否获取并替换Token？", default=False):
            ...
            return
        if Config.USER_KEY == "":
            Config.USER_KEY = input("请输入你的用户key: ")
            if not Config.USER_KEY:
                logger.error("用户key不能为空")
                logger.error("请填写你的用户key,找作者免费获取 https://gitee.com/tackok/free_cursor")
                return
        # 获取token数据
        token_data = TokenManager.fetch_token_data(Config.USER_KEY)
